...
def search_similar_compounds_chembl(
    query_smiles: str, similarity_threshold: int = 70, limit: int = 20
) -> List[Dict]:
    """
    ...
    try:
        ...
        results = similarity.filter(
            smiles=query_smiles, similarity=similarity_threshold
        ).only("molecule_chembl_id", "pref_name", "molecule_structures")[:limit]
        ...
        for compound in results:
            try:
                ...
                if compound.get("molecule_structures") and compound[
                    "molecule_structures"
                ].get("canonical_smiles"):
                    compound_smiles = compound["molecule_structures"][
                        "canonical_smiles"
                    ]
...
def compare_smiles_list(query_smiles: str, smiles_list: List[str]) -> List[Dict]:
    ...
    for smiles in smiles_list:
        ...
        if mol:
            ...
            results.append({"smiles": smiles, "similarity": round(similarity, 3)})
...
def get_smiles_from_name(name: str) -> Optional[str]:
    """
    Resolve a compound name to a canonical SMILES using the ChEMBL client.

    Args:
        name: Compound name (partial or full)

    Returns:
        Canonical SMILES string if found, otherwise None
    """
    try:
        # Use case-insensitive contains search on preferred name
        molecules = new_client.molecule.filter(pref_name__icontains=name).only(
